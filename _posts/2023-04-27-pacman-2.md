...
And so it happened - our programmers received a bug report stating that the DFS algorithm is unable to find a direct path from 0 to -5 on a line in example_3.
```c++
namespace example_1 { 
...
Nodes visited 251
```
Here  we have [halting problem](https://en.wikipedia.org/wiki/Halting_problem) and as far as there is no way to define autonomously when to stop function we will ask our user to define it for us.
First define concept 
```c++
template <typename F>
